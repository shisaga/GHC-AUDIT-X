...
import GHCToSOTable from "./GHCToSOTable";
...
const GHCToSO = ({
  ...
  const handleBatchInvoice = async () => {
    ...
    if (checkAuditorUniq.size == 1) {
      ...
      closeToast(toastId);
      errorToast(
        "Please make sure all selected So are the same in Invoice Summary generation"
      );
    }
    setDisableState(false)
  };
  ...
  useEffect(() => {
    refetch();
  }, [filter, currentPage, sortType, limitPerPage, selectedSO]);
  ...
  return (
    <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center justify-end gap-2 xs:gap-4">
              ...
              {!isTablet && (
                <SBSelect
                  ...
                  optionLabel ="Service Organization"
                  value={selectedSO || ""}
                  options={SOList?.map((element: any) => {
                    return {
                      value: element.id,
      ...
      <Card className="flex flex-col">
        <div className="md:px-6 px-4 py-4 mt-4 justify-between items-center grid grid-cols-1 md:grid-cols-3 gap-3">
